...
const ENDPOINT = '/acquisitions';

describe('API', () => {
  beforeEach(async () => {
    const models = [Acquisition];
  ...
  describe(`Get ${ENDPOINT} Get By Id`, async () => {
    it('return the row by id', async () => {
      const response = await createOneRow(ENDPOINT, { code: uuidv4() });
    ...
    it('Error when try to get a acquisition when id is not a number', async () => {
  ...
  describe(`Post ${ENDPOINT}`, async () => {
    it('create a new row called created', async () => {
      ...
      await expect(typeof data).to.equal('object');
      await expect(data.code).to.equal('created');
      await expect(err).to.equal(undefined);
    });
    it('create 5 rows successfully', async () => {
      ...
      for (let index = 0; index < 5; index += 1) {
        promises.push(createOneRow(ENDPOINT, { code: uuidv4() }));
      }
  ...
  describe(`PUT ${ENDPOINT}/id`, () => {
    ...
    it('Error when try to update a acquisition when id is not a number', async () => {
